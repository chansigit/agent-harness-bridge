"""HARNESS=deepseek backend for harness_bridge.run_agent — DeepSeek Harness
...
Env:
  ...
  DSH_BIN            path to the built `dsh` CLI entrypoint (apps/cli/lib/bin.js
                      from a source build). Default: $SCRATCH/tools/deepseek-harness-src/
...
# and the model runs on with no application tools. We close streams ourselves
# (dsh exits, server task cancelled),
...
def _write_raw_attachment_plugin(dsh_home: str, dsh_bin: str) -> tuple[str, str]:
    """Create the no-sharp attachment store used on older-glibc hosts.
    ...
    plugin = Path(dsh_home) / "harness-bridge-attachment-raw.mjs"
...
def _render_patch(mcp_url: str, allowed_builtin: tuple[str, ...], provider: str, model: str | None,
    ...
    insert: list[dict] = [
        {
            ...
            "config": {"serverName": "harness-bridge", "transport": "streamable-http", "url": mcp_url,
                       "failOnStartupError": True},
        },
        {
            "id": "harness-bridge-attachments",
    ...
    if provider != "deepseek-official":
        ...
        insert.append({
            "id": "harness-bridge-llm-provider",
            ...
            "config": {"providers": {provider: {
                ...
                # the host Read tool's admission limits.
...
def _run_sync(*, dsh_bin: str, cwd: str, dsh_home: str, provider: str, model: str | None,
    ...
    with DeepSeekHarness(
        ...
        try:
            ...
            # will do the remaining items") with no tool call — dsh reports the turn
